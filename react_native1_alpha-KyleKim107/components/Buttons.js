    ...
    return (
      <View style={containerStyle}>
        ...
        <Animated.View style={[textContainer, this.position.getLayout()]}{...this.panResponder.panHandlers}>
          <Text style={styles.textStyle}>{this.parseActivity()}</Text>
          <Text style={{marginTop: 5, fontSize: 18, color: "#38ADE1"}}>   {this.parseDuration()} min   {this.parseCalorie()} Calories</Text>
          <Text style={{fontWeight: 'bold',fontSize: 15, color: "grey"}}>{this.parseDate()}</Text>
        </Animated.View>
        {/* LEFT */}
